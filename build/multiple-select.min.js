...
scope.onSuggestedItemsClick(filteredSuggestionArr[scope.selectedItemIndex]);}};scope.onSuggestedItemsClick=function(selectedValue){if(scope.isSingleChoose){scope.modelArr=selectedValue;scope.isHover=false;scope.isFocused=false;}else{scope.modelArr.push(selectedValue);}
scope.inputValue="";};var isDuplicate=function(arr,item){var duplicate=false;if(arr==null||arr=="")
return duplicate;for(var i=0;i<arr.length;i++){duplicate=angular.equals(arr[i],item);if(duplicate)
break;}
return duplicate;};scope.alreadyAddedValues=function(item){var isAdded=true;isAdded=!isDuplicate(scope.modelArr,item);return isAdded;};scope.removeAddedValues=function(item){if(scope.isSingleChoose){scope.modelArr=null;}else{if(scope.modelArr!=null&&scope.modelArr!=""){var itemIndex=scope.modelArr.indexOf(item);if(itemIndex!=-1)
scope.modelArr.splice(itemIndex,1);}}};scope.mouseEnterOnItem=function(index){scope.selectedItemIndex=index;};function getRepeatItemsInArray(arr,column){var newArr=[];var repetitionArr=[];var i=0;if(column){for(i=0;i<arr.length;i++){if(newArr.indexOf(arr[i][column])==-1){newArr.push(arr[i][column])}else{if(repetitionArr.indexOf(arr[i][column])==-1){repetitionArr.push(arr[i][column])}}}}else{for(i=0;i<arr.length;i++){if(newArr.indexOf(arr[i])==-1){newArr.push(arr[i])}else{if(repetitionArr.indexOf(arr[i])==-1){repetitionArr.push(arr[i])}}}}
